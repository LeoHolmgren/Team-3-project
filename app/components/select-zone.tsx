'use client';

import { MutableRefObject } from 'react';
import { BiddingZone } from '@/app/types';
import { HomeState } from '@/app/home-context';
import { LocationSelectZone } from '@/components/select-zone-location';
import { SelectZonesDropdown } from '@/components/select-zone-dropdown';
import { BIDIGIT } from '@/app/constants';

import { SelectZoneController, useSelectZoneContext } from '@/components/select-zone-context';

export function SelectZone({
  homeState,
  onSelectZone,
  controllerRef,
}: {
  homeState: HomeState;
  onSelectZone: (zone: BiddingZone) => void;
  controllerRef: MutableRefObject<SelectZoneController | null>;
}) {
  // Context for this component
  const [state, controller, refs] = useSelectZoneContext(onSelectZone);

  // Pass controller reference to caller
  controllerRef.current = controller;

  const dropdown_btn = (
    <div className="h-[3.5em] w-full">
      <div
        className={
          'text[#5A5A5A] flex h-full w-full cursor-pointer items-center justify-between whitespace-nowrap rounded-md border border-input bg-background p-[0.5em] text-[1em] text-sm font-medium leading-[1] shadow-sm transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 dark:text-[#a3a3a3]' +
          (state.zoneLoaded ? ' !border-[hsl(var(--highlight))]' : '')
        }
      >
        <div className="font-[600] text-[#555]">{homeState.zone ? homeState.zone.value : 'ZONE'}</div>
        {homeState.zone ? homeState.zone.label : 'Select Zone'}
        <div className="font-[600] text-[#555]">
          {homeState.timeOfFetch
            ? BIDIGIT.format(homeState.timeOfFetch.getHours()) +
              ':' +
              BIDIGIT.format(homeState.timeOfFetch.getMinutes())
            : '--:--'}
        </div>
      </div>
    </div>
  );

  return (
    <div className="flex w-[100%] max-w-[406px] gap-[5px] text-[14px]">
      <LocationSelectZone controllerRef={refs.location} onSelectZone={controller.setBiddingZoneKeepLocation} />
      <SelectZonesDropdown onSelectZone={controller.setBiddingZone}>{dropdown_btn}</SelectZonesDropdown>
    </div>
  );
}
